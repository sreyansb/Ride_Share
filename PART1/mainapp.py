...
class CreateRide(Resource):
	def post(self):
		req_json = request.get_json()
		created_by = req_json["created_by"]
...
class RideApis(Resource):
	...
	def post(self, ride_id):
		...
		# have to give quotes around username or string types
		...
		msgbody = {"table": "rideuser", "column": [
			"rideid", "username"], "where": wherecond}
		reply = requests.post(
			"http://127.0.0.1:5000/api/v1/db/read", json=msgbody)
		ans = reply.json()
		if ans:
			...
			msgbody = {"table": "rideuser", "column": [
				"rideid", "username"], "insert": [ride_id, username]}
			reply = requests.post(
				"http://127.0.0.1:5000/api/v1/db/write", json=msgbody)
